...
def backprop(labels, theta, z, h, g, ry, cy, y, rx, cx, x):
    ...
    # for flatten backprop
    # we want this to work for any image size
    # 4*4*2(4) = 128
    # 2*4^3 = 128
    # 4 = (128/2)^(1/3)
    dim_size = int(np.cbrt(h.shape[1]/2))
    ...
    for i in range(N):
        ...
        p_h[i, :] = np.matmul(np.transpose(f_W), p_z)
        
        temp = np.zeros(f_W.shape)
    ...
    p_g = p_h.reshape((N, -1, dim_size, dim_size))
    ...
    p_cy = np.multiply(np.heaviside(cy, 0), p_ry)
    ...
    p_cx = np.multiply(np.heaviside(cx, 0), p_rx)
